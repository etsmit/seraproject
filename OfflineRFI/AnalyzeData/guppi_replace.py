...
#pulls from in_dir directory if full path not given
if sys.argv[1][0] != '/':
	sk_in0 = '/home/scratch/esmith/RFI_MIT/testing/' + sys.argv[2]
else:
	sk_in0 = sys.argv[2]
...
#pulls from in_dir directory if full path not given
if sys.argv[1][0] != '/':
	sk_in1 = '/home/scratch/esmith/RFI_MIT/testing/' + sys.argv[3]
else:
	sk_in1 = sys.argv[3]
...
for block in range(numblocks):
	...
	if method == 'zeros':
		...
		data[:,:,0] = repl_zeros(data[:,:,0],repl_chunk_p1,SK_ints,0)
		data[:,:,1] = repl_zeros(data[:,:,1],repl_chunk_p2,SK_ints,1)

	if method == 'previousgood':
		#replace data with previous (or next) good
		data[:,:,0] = previous_good(data[:,:,0],repl_chunk_p1,SK_ints,0)
		data[:,:,1] = previous_good(data[:,:,1],repl_chunk_p2,SK_ints,1)

	if method == 'stats':
		#replace data with statistical noise derived from good datapoints
		data[:,:,0] = statistical_noise(data[:,:,0],repl_chunk_p1,SK_ints,0)
		data[:,:,1] = statistical_noise(data[:,:,1],repl_chunk_p2,SK_ints,1)
