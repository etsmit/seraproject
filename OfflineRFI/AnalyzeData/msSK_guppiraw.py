#--------------------------------------------------
#msSK_guppiraw.py
#Opens GUPPI raw file, performs SK, gibs flags, copies and replaces data
#Multiscale SK variant (see 'A wideband Spectrometer with RFI detection' - Gary 2010)
#Takes inputs:
#1: Guppi raw file
...
ms_m = float(sys.argv[6][0])
ms_n = float(sys.argv[6][1])
...
for block in range(numblocks):
	...
	#Calculations of s1,s2 for later
	for j in range(num_pol):
		...
		for k in range(SK_timebins):
			...
			#compute S1
			s1 = np.sum(data_chunk,axis=1)

			#compute S2
			s2 = np.sum(data_chunk**2,axis=1)
		

			#append to results
			if j:
				s1_results_p2.append(spectrum)
				s2_results_p2.append(spectrum)
			else:
				s1_results_p1.append(spectrum)
				s2_results_p1.append(spectrum)


#Now we have a 2D array of S1 and S2 values for each polarization 
#- now need to rerun the same for loop, but this time calculating SK and flagging and replacing.


for block in range(numblocks):
	print('#--------------------------------------')
	print('Block: '+str(block))
	print('Calculating SK and Flags')
	if block == 0:
		header,headersize = rawFile.read_header()
		print('Header size: {} bytes'.format(headersize))
	header,data = rawFile.read_next_data_block()
	...
	for j in range(num_pol):
		...
		for k in range(SK_timebins):
			...
			#take the correct spectrum from S1 and S2
			spectrum = (block*SK_timebins)+k 
						s1_results_p2[]


			#This needs to be changed to accomodate MS-SK -----------	
			#perform SK
